...
class Plot:
  ...
  @staticmethod
  def animate_plot_multi(x_ax,y_ax,X_lab,Y_lab,type_func,plot_title,leg,loca,plot_col,direc,freq):
    x_vals = {}
    y_vals = {}
...
class Model:
  ...
  def plot(self,type_func,animate=False,direc=None):
    ...
    if (len(itera) > 100) and (len(itera) <= 1000):
      ...
      freq = 1
    plot_dict = {"Cost":[Plot.cost_static,Plot.animate_plot_multi,self.cost_tr_arr,self.cost_cv_arr,'Costs','Train-Cross Val Costs Curve','upper right',['c','#9ef705'],Plot.animate_plot,"Costs","Cost Function Curve"],
                 "Accuracy":[Plot.tr_cv_static,Plot.animate_plot_multi,self.accu_tr_arr,self.accu_cv_arr,'Accuracy','Train-Cross Val Accuracy Curve','lower right',['m','r'],Plot.animate_plot,"Accuracy","Accuracy Curve"]}
    if animate is False:
      plot_dict[type_func][0](self.cost_tr_arr,self.cost_cv_arr,self.accu_tr_arr,self.accu_cv_arr)
    else:
      if len(self.cost_cv_arr) != 0:
        plot_dict[type_func][1]([itera,itera],[plot_dict[type_func][2],plot_dict[type_func][3]],'Number of Iterations',plot_dict[type_func][4],type_func,plot_dict[type_func][5],['Train', 'Cross Val'],plot_dict[type_func][6],plot_dict[type_func][7],direc,freq)
      else:
...
model.fit(X_tr,y_tr,0.005,35,"Adam","Binary",mb=y_tr.shape[1],alpha_decay=True)
...
model.plot("Cost",animate=True,direc='/content/drive/My Drive/Colab Notebooks/nn_visuals/costs/costs3')

model.plot("Accuracy",animate=True,direc='/content/drive/My Drive/Colab Notebooks/nn_visuals/costs/accuracy3')
...
GRID_Y_START = 1.5
GRID_Y_END = -1
OUTPUT_DIR = "/content/drive/My Drive/Colab Notebooks/nn_visuals/moons_foo"
...
predictions_te = Helpers.predict_multi(act_te["A" + str(len(model1.layer_names))])
